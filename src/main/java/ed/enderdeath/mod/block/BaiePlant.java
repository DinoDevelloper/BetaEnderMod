...
public class BaiePlant extends Block implements IPlantable
{
    ...
    public BaiePlant(Item item)
    {
        super(Material.leaves);
        setTickRandomly(true);
        setHardness(0.0F);
        setBlockName("BaiePlant");
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int par6, float par7, float par8, float par9)
    {
        ...
        if(meta >= 1)
        {
            ...
            {
                world.setBlock(x, y, z, this, 0, 3);
                world.spawnEntityInWorld(new EntityItem(world, x, y, z, new ItemStack(Enderdeath.baie)));
                return true;
            }
        }
        else
    ...
    {
        return Enderdeath.baiePlantItem;
    }
    ...
    public void registerBlockIcons(IIconRegister icon)
    {
        ...
        for(int i = 0; i < fastIcons.length; i++)
        {
            if(textureNames[i] != "")
            {
                ...
                this.icon = icon.registerIcon(Enderdeath.MODID + ":Bush2Fast");
            }
        }
